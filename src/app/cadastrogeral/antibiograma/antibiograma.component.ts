...
@Component({
    selector: 'ms-antibiograma',
    templateUrl: './antibiograma.component.html',
    styleUrls: ['./antibiograma.component.scss'],
    providers: [AntibiogramaServices]
})
export class AntibiogramaComponent implements OnInit {
    public rowAntibiograma: any;
    ...
    fileNameDialogRef: MatDialogRef<AntibiogramaDialogComponent>;

    constructor(public api: AntibiogramaServices,
                private translate: TranslateService,
                private pageTitleService: PageTitleService,
                private dialog: MatDialog,
                private snackBar: MatSnackBar) {
        this.pageTitleService.setTitle('Antibiograma');
        const params = {
            skipHeader: false,
            skipFooters: true,
            skipGroups: true,
    ...
    openFileDialog(file?) {
        if (file) { // Editando
            this.fileNameDialogRef = this.dialog.open(AntibiogramaDialogComponent, {
                height: '400px',
                width: '600px',
                // position: {
                //     'top': '15%',
        ...
        this.fileNameDialogRef.afterClosed().pipe(
            ...
            if (this.rowAntibiograma) {
                const index = this.rowAntibiograma.findIndex(f => f.descricao === file);
                if (index !== -1) {
                    this.rowAntibiograma[index] = {descricao, content: file};
                } else {
                    this.rowAntibiograma.push({descricao, content: ''});
                }
            }
            this.ngOnInit();
        });
